...
import { format } from "date-fns"; // Removed unused subDays import
...
export default function AdminAnalyticsPage() {
  ...
  // Generate comparison data using useMemo to avoid Math.random during render
  const comparisonData = useMemo(() => {
    if (!usageData) return [];

    // Use a deterministic multiplier instead of random
    return usageData.map((item) => ({
      ...item,
      previous: Math.floor(item.count * 0.9), // 90% of current as mock previous data
    }));
  }, [usageData]);
